...
function generarPDF(datos) {
  ...
  doc.text('COMPROBANTE DE INSCRIPCIÓN', pageWidth / 2, 30, { align: 'center' });
  ...
  doc.text('Gracias por inscribirte a nuestro torneo. A continuación encontrarás los detalles de tu inscripción:', margin, 60);
  ...
  yPos += 7;
  doc.text(`Edad: ${datos.edad} años`, margin, yPos);
  yPos += 7;
  doc.text(`Teléfono: ${datos.telefono}`, margin, yPos);
  yPos += 7;
  doc.text(`Dirección: ${datos.direccion}`, margin, yPos);
  yPos += 15;
  ...
  doc.setFont(undefined, 'normal');
  doc.text(`Sede: ${datos.sede}`, margin, yPos);
  yPos += 7;
  doc.text(`Fecha y Horario: ${datos.fecha}`, margin, yPos);
  yPos += 15;
  ...
  doc.text('ROL DUNGEON - www.roldungeon.com', pageWidth - margin, 280, { align: 'right' });
